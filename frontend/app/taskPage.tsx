...
function addDays(base:Date,n:number){return new Date(base.getFullYear(),base.getMonth(),base.getDate()+n)}
...
export default function TaskPage(){
  ...
  const loadChores=useCallback(async()=>{try{await Promise.all([apiGetWithBody('/chores/house',{house_id:API_HOUSE_ID}),apiGetWithBody('/chores/user',{user_id:API_USER_ID})])}catch{}},[])
  ...
  return(
    <GradientBackground>
      <SafeAreaView style={s.safe}>
        ...
        <ScrollView contentContainerStyle={s.scroll} showsVerticalScrollIndicator={false}>
          <ScrollView horizontal showsHorizontalScrollIndicator={false} contentContainerStyle={s.dateRow}>
            {dateOptions.map(d=>{
              ...
              return(
                <Pressable key={d.id} onPress={()=>setSelectedId(d.id)} style={({pressed})=>[s.datePill,on?s.datePillOn:s.datePillOff,pressed&&s.pressed]}>
          ...
          <ScrollView horizontal showsHorizontalScrollIndicator={false} contentContainerStyle={s.filterRow}>
            {FILTERS.map(f=>(
              <Pressable key={f} onPress={()=>setFilter(f)} style={({pressed})=>[s.pill,filter===f&&s.pillOn,pressed&&s.pressed]}>
                ...
                <GlassCard key={task.id} style={s.taskCard}>
                  ...
                  <View style={s.cardBottom}>
                    ...
                    <View style={s.statusRow}>
                      ...
                      <Pressable onPress={()=>taskStore.toggleDone(task.id)} style={[s.doneCircle,task.done?s.doneOn:s.doneOff]}>
        ...
        {/* FAB — rectangular, F2E8DC, no border around + */}
