...
export const handler = async (
  ...
  const { expense_id, ...updates } = JSON.parse(event.body);
  ...
  const allowedFields = ["name", "price", "due_date", "is_urgent", "owers", "payers"];
  ...
  return {
    statusCode: 200,
    body: JSON.stringify({ message: "Expense updated" }),
  };
};
