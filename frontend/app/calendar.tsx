  ...
  return (
    <GradientBackground>
      <SafeAreaView style={s.safe}>
          ...
          <View style={s.dateStrip}>
            {dateOptions.map(d => {
              ...
              return (
                ...
                >
                  {/* month + weekday use same font as Day View toggle */}
          ...
          {viewMode === 'Month View' && (
            <GlassCard style={s.monthCard}>
              ...
              <View style={s.calGrid}>
                {Array.from({length:firstDay(calYear,calMonth)},(_,i)=><View key={`e${i}`} style={s.calCellBlank}/>)}
                {Array.from({length:daysIn(calYear,calMonth)},(_,i)=>{
                  ...
                  return(
                    <TouchableOpacity key={day} style={[s.calCell,sel&&s.calCellOn]}
                      onPress={()=>{setSelectedId(key);setViewMode('Day View')}}>
                      <Text style={[s.calCellTxt,sel&&s.calCellTxtOn]}>{day}</Text>
                      {hasTask(calYear,calMonth,day)&&<View style={[s.taskDot,sel&&s.taskDotOn]}/>}
                    </TouchableOpacity>
                  )
                })}
              </View>
        ...
        {/* Add Task FAB — rectangular, text F2E8DC, icon F2E8DC, no border */}
        <Pressable
...
const s = StyleSheet.create({
  ...
  // Tall date cards
  dateStrip:  { flexDirection:'row', gap:7, marginBottom:16 },
  dateTall:   { flex:1, borderRadius:18, alignItems:'center', paddingTop:14, paddingBottom:14, minHeight:110, justifyContent:'space-between', borderWidth:1 },
  dateTallOn: { backgroundColor:`rgba(236,133,117,0.70)`, borderColor:`rgba(236,133,117,0.70)` },
  dateTallOff:{ backgroundColor:PALETTE.glass, borderColor:PALETTE.glassBorder },
  // month/weekday: same font as mode buttons (Lexend Deca)
  ...
  modeBtnOn:     { backgroundColor:`rgba(236,133,117,0.70)`, borderColor:`rgba(236,133,117,0.70)` },
  ...
  calCellOn:    { backgroundColor:`rgba(236,133,117,0.70)` },
  ...
  // FAB — rectangular, no border, F2E8DC text & icon
